...
| CRMAAREG-129 / F6 | Under arbeid           | Delvis                                            | `CorrelationContext` og `IntegrationLogContext` med testar; `IntegrationLogRedactor` fjernar sensitive feltverdiar (token, secret, authorization, cookie, fnr, ssn) før logging, dokumentert i P360 teknisk oversikt; `P360_IntegrationException.withCorrelationId` fører correlation-ID gjennom eit catch-grense. Verifisert mot scratch org `crm-arbeidsforhold`: deploy `0AfQI00000jKxV30AK` (2/2) og deploy `0AfQI00000jKxmn0AC` (4/4), alle testar grøne | Strukturert logger (faktisk skriving/persistens av loggoppføringar) og transportpropagering over HTTP gjenstår; sistnemnde er avhengig av #1017 (RPC-miljø/endepunkt/auth) |
