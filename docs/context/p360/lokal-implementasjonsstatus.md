...
| CRMAAREG-129 / F6 | Under arbeid           | Delvis                                            | `CorrelationContext` og `IntegrationLogContext` med testar; ny `IntegrationLogRedactor` fjernar sensitive feltverdiar (token, secret, authorization, cookie, fnr, ssn) før logging. Verifisert mot scratch org `crm-arbeidsforhold`: deploy `0AfQI00000jKxV30AK`, 2/2 testar og 100 % dekning | Strukturert logger (faktisk skriving av logg), transportpropagering og persistens gjenstår |
