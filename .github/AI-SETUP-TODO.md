...
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: legacy `test`-prefikserte testmetodar er **repo-omfattande** (186 metodar i 21 testklassar), ikkje avgrensa til dei to klassane som først blei nemnde. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under. Ingen rename eller ApexDoc er lagt til i denne kartlegginga; det er eige, seinare arbeid med eiga TDD-/review-skive per klasse.
...
- [ ] Attståande naming-/kodekvalitetsoppgåver (basert på kartlegginga og ApexDoc-arbeidet): (1) avklare namnekonvensjonsbrota, inkludert produksjonsnamnet `AAREG_checkObjectTypeNameche` og dei 186 legacy-testnamna, (2) vurdere dei nyleg avdekte Sev2 CRUD/FLS-funna i `AAREG_HomeController` og andre klassar der blanket-PMD-undertrykking tidlegare skjulte reelle funn. Kvar av desse skal takast som eiga, avgrensa TDD-/review-skive; ikkje masse-rename utan migreringsplan, og `force-app/unpackagable/` skal ikkje endrast utan eksplisitt godkjenning.

### Første kartleggingsfunn

- Legacy testmetodenamn med `test`-prefiks (t.d. `testGetOrganizationsWithRoles`) finst i 186 metodar fordelt på 21 testklassar over heile `force-app/`, inkludert `AAREG_HomeControllerTest`, `AAREG_ApplicationControllerTest`, `AAREG_AgreementTest`, `AAREG_MyAgreementsControllerTest`, `AAREG_distributionAccessControllerTest`, `AAREG_distributionAccessRequestTest`, `AAREG_ApplicationInternalControllerTest`, `AAREG_ApplicationDecisionControllerTest`, `AAREG_ApplicationDecisionPDFCtrlTest`, `AAREG_ApplicationDecisionPDFFileSerTest`, `AAREG_ApplicationDecisionTest`, `AAREG_CreateContentAgreementTest`, `AAREG_DeactivateAccessBatchTest`, `AAREG_CheckObjectTypeNamingTest`, `AAREG_MultiSelectFlowValuesTest`, `AAREG_CodeBaseListControllerTest`, `AAREG_ContactSupportControllerTest`, `AAREG_AgreementControllerTest`, `DecisionControllerTest`, og dei read-only `ChangePasswordControllerTest`/`CommunitiesLandingControllerTest` i `unpackagable/`.
