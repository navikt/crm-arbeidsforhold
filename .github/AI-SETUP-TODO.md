...
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: legacy `test`-prefikserte testmetodar var **repo-omfattande** (186 metodar i 21 testklassar), ikkje avgrensa til dei to klassane som først blei nemnde. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under. Seinare er 34 testmetodar i `AAREG_HomeControllerTest`, `AAREG_CodeBaseListControllerTest`, `DecisionControllerTest`, `AAREG_MultiSelectFlowValuesTest` og `AAREG_AgreementTest` renamde etter åtferd; 149 legacy-namn står att.
