...
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: legacy `test`-prefikserte testmetodar var **repo-omfattande** (186 metodar i 21 testklassar), ikkje avgrensa til dei to klassane som først blei nemnde. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under. Seinare er 180 testmetodar i dei redigerbare testklassane renamde etter åtferd; 11 legacy-namn står att, der 9 er redigerbare og 2 ligg i read-only `unpackagable/`.
...
- [x] Renamde 180 legacy-testmetodar i dei redigerbare testklassane til åtferdsbaserte namn. Prettier passerer, og ingen `test`-prefiks står att i dei rydda klassane. Editorvalidering er framleis org-avhengig på grunn av manglande Salesforce-/custom-object-typar. `force-app/unpackagable/` er ikkje endra.
