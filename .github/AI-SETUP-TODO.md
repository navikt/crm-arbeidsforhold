...
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: den historiske kartlegginga fann 186 `test`-prefikserte metodar i 21 testklassar; etter sletting av ein redundant testklasse og dei åtferdsbaserte renamingsskivene viser live-søket no 4 attståande metodar. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under.
...
- [x] Renamde 181 legacy-testmetodar i dei redigerbare testklassane til åtferdsbaserte namn. Live-søket viser 2 redigerbare og 2 read-only metodar att. Prettier passerer, og ingen `test`-prefiks står att i dei rydda klassane. Editorvalidering er framleis org-avhengig på grunn av manglande Salesforce-/custom-object-typar. `force-app/unpackagable/` er ikkje endra.
