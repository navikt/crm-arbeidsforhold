...
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: legacy `test`-prefikserte testmetodar var **repo-omfattande** (186 metodar i 21 testklassar), ikkje avgrensa til dei to klassane som først blei nemnde. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under. Seinare er 26 testmetodar i `AAREG_HomeControllerTest`, `AAREG_CodeBaseListControllerTest`, `DecisionControllerTest` og `AAREG_MultiSelectFlowValuesTest` renamde etter åtferd; 157 legacy-namn står att.
...
- [x] Renamde 26 legacy-testmetodar i `AAREG_HomeControllerTest`, `AAREG_CodeBaseListControllerTest`, `DecisionControllerTest` og `AAREG_MultiSelectFlowValuesTest` til åtferdsbaserte namn. Prettier passerer, og ingen `test`-prefiks står att i klassane. Editorvalidering er framleis org-avhengig på grunn av manglande Salesforce-/custom-object-typar.
