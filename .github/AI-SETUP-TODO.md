...
- [x] Vurdert lokal secret scanning med Gitleaks: stadfesta at GitHub sin native secret scanning og push protection alt er aktivert for repoet. Push protection stoppar hemmelegheiter før dei blir pusha, som er sterkare enn ein CI-tids Gitleaks-skanning. Vilkåret for å innføre Gitleaks («dersom CI ikkje allereie dekkjer behovet») er difor ikkje oppfylt; ikkje lagt til no.
- [x] Gjennomført naming-/ApexDoc-kartlegging av `force-app/` (les-only). Korrigert og utvida det tidlegare funnet: legacy `test`-prefikserte testmetodar var **repo-omfattande** (186 metodar i 21 testklassar), ikkje avgrensa til dei to klassane som først blei nemnde. Fann i tillegg 13 offentlege produksjonsklassar utan ApexDoc og 6 klassar som bryt PascalCase-etter-prefiks-konvensjonen. Full liste er i «Første kartleggingsfunn» under. Seinare er 84 testmetodar i `AAREG_HomeControllerTest`, `AAREG_CodeBaseListControllerTest`, `DecisionControllerTest`, `AAREG_MultiSelectFlowValuesTest`, `AAREG_AgreementTest`, `AAREG_distributionAccessControllerTest`, `AAREG_distributionAccessRequestTest`, `AAREG_ApplicationInternalControllerTest`, `AAREG_CreateContentAgreementTest`, `AAREG_DeactivateAccessBatchTest` og `AAREG_ApplicationDecisionTest` renamde etter åtferd; 99 legacy-namn står att.
- [x] Lagt til ApexDoc på alle 14 produksjonsklassane frå kartlegginga (`AAREG_HomeController`, `AAREG_CodeBaseListController`, `AAREG_MyThreadsController`, `AAREG_MyApplicationsController`, `DecisionController`, `AAREG_DistributionAccessMock`, `AAREG_AccessRequestTasksController`, `AAREG_ApplicationInternal`, `AAREG_ApplicationInternalController`, `AAREG_BasisCodesInternal`, `AAREG_RelatedContactInternal`, `AAREG_createContentVersionForAgreement`, `AAREG_DeactivateAccessOnDeletedOrgsBatch`, `AAREG_TestDataFactory`), etter den offisielle Salesforce ApexDoc-standarden. `@author`/`@since` er henta frå verifisert `git log --follow`-historikk (original forfattar først, deretter topp-3 etter commit-tal i kronologisk rekkefølgje), aldri gjetta. Retta eit format-avvik i `AAREG_MyApplicationsController` der ApexDoc stod etter i staden for før `@AuraEnabled`-annotasjonen, og fjerna ein feilplassert ApexDoc-liknande kommentar inni ein metodekropp. Fjerna blanket `@SuppressWarnings('PMD')` frå 5 filer der det fanst. Dette avdekte reell, aktiv PMD/Code Analyzer-diagnostikk i editoren, inkludert fleire Sev2 CRUD/FLS-tryggingsfunn i `AAREG_HomeController` som tidlegare var skjulte; desse er reelle kodekvalitetsfunn utanfor ApexDoc-oppgåva og er ikkje retta her. Ingen rename er gjort; namnekonvensjonsbrota og legacy-testnamna står framleis att som eiga oppgåve.
- [x] Lagt til ApexDoc på alle 28 testklassane i `force-app/`. Kvar klasse har verifiserte `@author`, `@since` og `@group`-taggar, og kvar testmetode har `@since` for å synleggjere når testen blei introdusert. Forfattarar og datoar blei henta frå full `git log --follow -p`-historikk, med original forfattar først og utan gjetting. Alle endra filer er validerte med `get_errors` og `npx prettier --write`; attståande funn er eksisterande org-avhengige/PMD-diagnosar, ikkje ApexDoc-feil.
- [x] Renamde 84 legacy-testmetodar i `AAREG_HomeControllerTest`, `AAREG_CodeBaseListControllerTest`, `DecisionControllerTest`, `AAREG_MultiSelectFlowValuesTest`, `AAREG_AgreementTest`, `AAREG_distributionAccessControllerTest`, `AAREG_distributionAccessRequestTest`, `AAREG_ApplicationInternalControllerTest`, `AAREG_CreateContentAgreementTest`, `AAREG_DeactivateAccessBatchTest` og `AAREG_ApplicationDecisionTest` til åtferdsbaserte namn. Prettier passerer, og ingen `test`-prefiks står att i klassane. Editorvalidering er framleis org-avhengig på grunn av manglande Salesforce-/custom-object-typar.
...
- [ ] Attståande naming-/kodekvalitetsoppgåver (basert på kartlegginga og ApexDoc-arbeidet): (1) avklare namnekonvensjonsbrota, inkludert produksjonsnamnet `AAREG_checkObjectTypeNameche` og dei 99 attståande legacy-testnamna, (2) vurdere dei nyleg avdekte Sev2 CRUD/FLS-funna i `AAREG_HomeController` og andre klassar der blanket-PMD-undertrykking tidlegare skjulte reelle funn. Kvar av desse skal takast som eiga, avgrensa TDD-/review-skive; ikkje masse-rename utan migreringsplan, og `force-app/unpackagable/` skal ikkje endrast utan eksplisitt godkjenning.

### Første kartleggingsfunn

- Legacy testmetodenamn med `test`-prefiks står att i 170 metodar fordelt på dei opphavlege testklassane over heile `force-app/`. `AAREG_HomeControllerTest` er ferdig rydda i denne skiva. Dei read-only `ChangePasswordControllerTest`/`CommunitiesLandingControllerTest` i `unpackagable/` skal framleis ikkje endrast.
- Offentlege produksjonsklassar utan ApexDoc (`/**`-blokk): `AAREG_CodeBaseListController`, `AAREG_MyThreadsController`, `AAREG_HomeController`, `AAREG_MyApplicationsController`, `DecisionController`, `AAREG_DistributionAccessMock`, `AAREG_AccessRequestTasksController`, `AAREG_ApplicationInternal`, `AAREG_ApplicationInternalController`, `AAREG_BasisCodesInternal`, `AAREG_RelatedContactInternal`, `AAREG_createContentVersionForAgreement`, `AAREG_DeactivateAccessOnDeletedOrgsBatch`, `AAREG_TestDataFactory`.
- Namnekonvensjonsbrot (skal vere PascalCase rett etter `AAREG_`-prefikset): `AAREG_accountSelector`, `AAREG_contactSupportController`, `AAREG_distributionAccessController`, `AAREG_distributionAccessRequest`, `AAREG_createContentVersionForAgreement`, `AAREG_checkObjectTypeNameche`. Sistnemnde har eit moglegvis feilstava suffiks. Den redundante testklassen `AAREG_CheckObjectTypeNamingTest` blei fjerna etter dependency-kontroll; produksjonsklassen og den bevarte testen er uendra.
