...
export default class Aareg_application extends NavigationMixin(LightningElement) {
    ...
    setBreadcrumbs() {
        if (
            this.currentPageReference.state.c__applicationType !== 'view' &&
            this.currentPageReference.state.c__applicationType !== 'edit'
        ) {
            this.isEdit = false;
            this.breadcrumbs = [
                {
    ...
    async checkAccessToApplication() {
        getUserRights({
            ...
            resourceName: 'nav_arbeidsforhold_aa-registeret-sok-tilgang'
        })
            .then((result) => {
                if (result.success) {
    ...
    handleSubmit(event) {
        ...
        this.isLoaded = false;
        const { base64, filename } = this.fileData;
        this.application.AA_changesInApplication__c =
            this.application.Status__c === 'Additional Information Required' ? true : false;
        processApplication({
            application: this.application,
            basisCode: this.applicationBasisRows,
    ...
    renderedCallback() {
        ...
        if (
            this.isLoaded === true &&
            this.template.querySelector('c-alertdialog') !== undefined &&
    ...
    checkApplicationInputs() {
        ...
        if (
            ...
            this.template.querySelector('c-record-files-with-sharing').checkIfEmpty() === true
        ) {
