...
| (none; project-specific setup scripts previously required forking the tool)  | `sf-project` `customPostSteps` config          | New capability      | A project can declare additional named post-steps as external commands, run after the built-in steps in configured order. |
| `bin/create-scratch-org.sh --skip-org --skip-packages` (`--post-steps-only`) | `sf-project project configure --skip-packages` | Implemented         | Runs only selected post-steps against an already-configured org; no org lifecycle or package command runs.                |     | Deploy stops on source-tracking conflicts against a pool org | `sf-project org create` / `project configure` | Corrected in both | Deploy uses `--ignore-conflicts` and tracking is reset before/after deploy in both the TypeScript CLI and the legacy `bin/create-scratch-org.sh` script, so a reused pool org never blocks metadata deployment. |     | Self-check | `sf-project doctor` | Implemented | Uses stable exit categories and actionable results. |
...
| Deterministic local package simulation                                       | `packages install/update/plan --mock`          | New capability      | Uses injected fixtures without Salesforce commands; normal `--dry-run` remains real read-only planning.                   |
| Core coverage reporting                                                      | `npm run coverage:core`                        | New capability      | V8 baseline is reported before a blocking threshold is introduced.                                                        |
