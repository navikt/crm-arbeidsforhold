...
Package installation submits asynchronously and polls `sf package install report` by request ID, so the local process does not remain blocked inside the Salesforce CLI's built-in `--wait` handling after the server has finished. Each poll also checks `sf package installed list`, which lets the operation finish as soon as the exact selected subscriber package version is visible even if the request report lags behind and still says `IN_PROGRESS`. If polling still times out, the same installed-package check is used as the final reconciliation; a failed or inconclusive status check remains a timeout failure, so the command does not blindly retry or reinstall a package whose final state is unknown.
