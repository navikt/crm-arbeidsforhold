...
| `packages.plan`        | `{targetOrg?: string, installLatest?: boolean, dryRun?: boolean, mock?: boolean, mockScenario?: string}`                                                                                                                                              |
| `packages.install`     | `{targetOrg?: string, installLatest?: boolean, dryRun?: boolean, mock?: boolean, mockScenario?: string}`                                                                                                                                              |
| `packages.update`      | `{targetOrg?: string, installLatest?: boolean, dryRun?: boolean, mock?: boolean, mockScenario?: string}`                                                                                                                                              |
...
For package commands, `mock: true` bypasses real-org preauthorization and routes execution to deterministic local fixtures. It never invokes `sf`/`sfp` or mutates an org. Supported package scenarios are `success`, `failure`, `timeout`, `retry`, and `partial`. This mock boundary is currently limited to package commands; org workflow and dependency-refresh mock support remain separate work.
