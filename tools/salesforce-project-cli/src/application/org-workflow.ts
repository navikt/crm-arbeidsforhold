...
async function runStep(
    options: Pick<ConfigureProjectOptions, 'configuration' | 'emit' | 'operationId' | 'runCommand' | 'signal'>,
    stepId: string,
    step: string,
    executable: string,
    commandArguments: string[]
): Promise<ExitCode> {
    return runCommandStep(options, stepId, step, executable, commandArguments);
}
